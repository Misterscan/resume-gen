...
def test_get_gdoc_text_success(mock_download, mock_build):
    ...
    assert text == "Test Google Doc Content"


@patch("services.google_drive.build")
@patch("services.google_drive.MediaFileUpload")
def test_upload_to_gdoc_create_new(mock_media_upload, mock_build):
    mock_drive_service = MagicMock()
    mock_build.return_value = mock_drive_service
...
@patch("services.google_drive.build")
@patch("services.google_drive.MediaFileUpload")
def test_upload_to_gdoc_overwrite_existing(mock_media_upload, mock_build):
    mock_drive_service = MagicMock()
    mock_build.return_value = mock_drive_service
    ...
    mock_files.update.return_value.execute.return_value = {
        "id": "existing_id",
        "webViewLink": "http://updated_link",
    }

    link = upload_to_gdoc(
        "/path/to/file.docx", "file.docx", "dummy_creds", overwrite_id="existing_id"
    )

    assert link == "http://updated_link"
